            ...
            if splitted_text[0] == "info":
                for n in range(len(splitted_text)):
                    if splitted_text[n] == "score":
                        principal_variation = " ".join(splitted_text[splitted_text.index("pv") + 1 :]) if "pv" in splitted_text else ""
                        evaluation = {
        ...
        for current_line in reversed(lines):
            if current_line[0] == "bestmove":
                ...
                if multiPV_number <= num_top_moves:
                    ...
                    principal_variation = " ".join(current_line[current_line.index("pv") + 1 :])
                    top_moves.insert(
                        0,
                        {
